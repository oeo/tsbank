...
import { AccountService, BITCOIN_WALLET_REQUIRED } from '../../../src/core/accounts/AccountService';
...
describe('AccountService', () => {
    ...
    it('should successfully withdraw from an account', async () => {
        ...
        spyOn(config, 'get').mockReturnValue({ ...config.get(), limits: { ...config.get().limits, withdrawal_limits: { checking: 10000, savings: 5000, bitcoin_collateral: 2000 } } });
    ...
    it('should throw an error if withdrawal amount exceeds limits', async () => {
        ...
        const account = Account.create('acc-123', 'cust-456', 'checking', initialBalance);
        (mockAccountRepo.findById as any).mockResolvedValueOnce(account);
        spyOn(config, 'get').mockReturnValue({ ...config.get(), limits: { ...config.get().limits, withdrawal_limits: { checking: 500, savings: 500, bitcoin_collateral: 500 } } });
