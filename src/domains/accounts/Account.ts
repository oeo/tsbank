    ...
    private apply(event: DomainEvent): void {
        switch (event.eventName) {
            case ACCOUNT_CREATED:
                ...
                this.balance = Money.from(event.payload.initialBalance.props);
                this.status = 'open';
                break;
            case ACCOUNT_DEPOSITED:
                this.balance = this.balance.add(Money.from(event.payload.amount.props));
                break;
            case ACCOUNT_WITHDRAWN:
                this.balance = this.balance.subtract(Money.from(event.payload.amount.props));
                break;
            case ACCOUNT_CLOSED:
                this.status = 'closed';
                break;
